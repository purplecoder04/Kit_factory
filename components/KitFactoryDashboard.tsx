...
import type { OutputMode, ParsedKit, ValidationIssue } from "@/lib/parser/pageTypes"
...
export function KitFactoryDashboard() {
  ...
  async function downloadOutput(kind: "render" | "fillable") {
    setIsWorking(true)
        ...
        <div className="grid gap-4 lg:grid-cols-[156px_1fr]">
          <div className="max-h-[430px] overflow-auto rounded-lg border">
            {pages.map((page, index) => (
              <div
                className="grid grid-cols-[30px_1fr] gap-2 border-b p-3 text-sm last:border-b-0"
                key={page.id}
              >
